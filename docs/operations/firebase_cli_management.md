...
> **⚠️ 注記（2026-04-05）**
> Firebase プロジェクト `ai-workshop-vote` は削除済みです。
> このドキュメントは経緯の記録として保持していますが、現在は稼働中のプロジェクトと紐づいていません。
> 将来 Firebase を再利用する場合は、新規プロジェクトを作成し `.firebaserc` を更新してから進めてください。
> 意思決定の詳細は `DECISIONS.md` の DEC-0004 を参照してください。
