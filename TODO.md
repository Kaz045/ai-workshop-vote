...
- [x] README.md を最新状態に更新
- [ ] **投票終了日（2026-03-13）当日の締め作業**
  - Firestore の最終結果を保存（エクスポートまたは手動記録）
  - 参加者向け案内から投票URLを外す
  - 必要なら `index.html` をメンテナンス表示に差し替える
- [ ] **セキュリティ対応：Git履歴に残存する Firebase 設定値の無効化と除去**
  ...
  - 実施タイミング: 投票回収完了後（2026-03-13 以降）に実施
...
| ローカルファイル | html_experiment/index.html（開発用）、index.html（GitHub Pages用・デバッグログ入り） |
